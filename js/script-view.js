    ...
    function com_view_target(param) {
        return new Vue({
            ...
            updated: function() {
                let width = 0;
                let w;
                $('.one-size').each((i, elem) => {
                    $(elem).find('.input-group-text').css({'font-weight': 900});
                    w = $($(elem).find('.input-group-text')[0]).width();
                    if (width < w) width = w;
                });
                $('.one-size').each((i, elem) => {
                    $($(elem).find('.input-group-text')[0]).width(width);
                });
            },
            methods: {
                            ...
                            if (this.head[h].dict) {
                                for (let it in item) {
                                    ...
                                    if (this.head[h].edit && this.head[h].edit != 'c-select-2') {
                                        ...
                                        item[it] = '<div class="border-left-secondary border bg-light ' +  st + ' font-weight-bold my-1 mr-1 px-1">' + item[it] + '</div>';
                                    }
                                }
                            }
                            item = item.join('');
                    ...
                    //  обработка ответа
                        if ('error' in e.res) {   //  ошибка
                            ...
                            if ('items' in e.res) {
                                ...
                                //  ссылка для excel
                                    this.excel_link = 'excel.php?type=v-1&FILTER=' + JSON.stringify(this.data.filter) + '&SEARCH=' + JSON.stringify(this.search);
                            }
            ...
            template: `
                <div class="pt-3">
                    ...
                    <div v-if="Object.keys(data.legend).length" class="mb-3 px-3 py-2 alert-secondary">
                        <b>{{ lang.com['012'] }}</b>
                    ...
                    <div class="table-responsive mb-3">
                        <table id="data-chart" class="table table-sm m-0 text-dark">
                            ...
                            <tbody>
                                ...
                                <template v-if="this.data.total_items">
                                    <template v-for="(item, i) in data.items">
                                        <!-- ◉◉◉◉◉ ПРЕДСТАВЛЕНИЕ ◉◉◉◉◉ -->
                                        <tr><td class="td-middle"></td></tr>
                                        <tr 
                                            :key="i"
                                            class="form-height"
                                            :class="'n' + Math.ceil(i % 2) + (item.id_val === null ? ' text-danger' : '')"
                                            :style="item.id_val !== null ? 'background-color: rgba(' + parseInt(data.legend[item.id_val].color.substring(0,2), 16) + ',' + parseInt(data.legend[item.id_val].color.substring(2,4), 16) + ',' + parseInt(data.legend[item.id_val].color.substring(4), 16) + ',0.3)': ''"
                                        >
                                            <template
                                                v-for="(h, hi) in head"
                                            ...
                                            >
                                                <td
                                                    v-if="h.data"
                                                    :class="h.view"
                                                    class="align-middle px-3 border-0"
                                                    :style="hi === 'id_target' ? 'border-color: #' + (item.id_val !== null ? data.legend[item.id_val].color : 'dc3545') + ' !important;' : ''"
                                                >
                    ...
                    //  обработка ответа
                        if ('error' in e.res) {   //  ошибка
                            ...
                            if ('items' in e.res) {
                                ...
                                //  ссылка для excel
                                    this.excel_link = 'excel.php?type=v-2&FILTER=' + JSON.stringify(this.data.filter);
                            }
                            else {  //  ошибка получения данных
                                this.error = this.lang.err[102];
                                this.$set(this.data, 'items', {});
                        ...
                        <table class="table table-sm table-hover m-0">
                            <tbody>
                                <tr v-for="item in data.items">
                                    <td class="border-0 w-20 align-middle py-2 pl-0 pr-2" style="line-height: 1.1rem" v-html="item.title"></td>
                                    <td class="border-0 align-middle w-100 px-0">
                                        <div class="d-flex align-items-center w-100">
                                            <template v-for="val in item.data">
                                                <div 
                                                    v-if="val.value != 0"
                                                    class="align-self-stretch py-1 text-nowrap text-truncate text-white px-1"
                                                    :class="{'bg-danger': val.id_val == '-'}"
                        ...
                        if ('error' in e.res) {   //  ошибка
                            ...
                            if ('items' in e.res) {
                                ...
                                //  обработка словарей
                                    ...
                                    this.dict_val = e.res.dict_val;
                                //  ссылка для excel
                                    this.excel_link = 'excel.php?type=v-3&FILTER=' + JSON.stringify(this.data.filter);
                            }
                            else {  //  ошибка получения данных
                                this.error = this.lang.err[102];
                                this.$set(this.data, 'items', {});
                ...
                <div class="pb-3">
                    ...
                    <div class="table-responsive pb-3">
                        <table class="table table-sm table-hover m-0">
                            <tbody>
                                <tr v-for="item in data.items">
                                    <td class="border-0 w-20 align-middle py-2 pl-0 pr-2 small" style="line-height: 1.1rem" v-html="item.title"></td>
                                    <td class="border-0 align-middle w-100 px-0">
                                        <div class="d-flex align-items-center w-100">
                                            <template v-for="val in item.data">
                                                <div 
...
//  компонент VIEW_TARGETS
    function com_view_total(param) {
        return new Vue({
            el: '#' + param['name'],
            data: {
                lang: param['lang'],
            ...
            computed: {
                ...
                filter_set: function() {
                    ...
                    for (let i in this.data.filter) {
                        ...
                        if ('id_val' in this.data.filter) if (this.data.filter['id_val'] == null) this.data.filter['id_val'] = null;
                        if ('id_level' in this.data.filter) if (this.data.filter['id_level'] == null) this.data.filter['id_field'] = null;
                        if ('id_field' in this.data.filter) if (this.data.filter['id_field'] == null) this.data.filter['id_class'] = null;
                        if ('id_class' in this.data.filter) if (this.data.filter['id_class'] == null) this.data.filter['id_item'] = null;
                        if ('id_item' in this.data.filter) if (this.data.filter['id_item'] == null) this.data.filter['id_sec'] = null;
                        ...
                        if ('error' in e.res) {   //  ошибка
                            ...
                            if ('items' in e.res) {
                                ...
                                //  обработка словарей
                                    ...
                                    //this.dict_val = e.res.dict_val;
                                //  ссылка для excel
                                    this.excel_link = 'excel.php?type=v-4&FILTER=' + JSON.stringify(this.data.filter);
                            }
                            else {  //  ошибка получения данных
                                this.error = this.lang.err[102];
                                this.$set(this.data, 'items', {});
            ...
            template: `
                <div>
                    ...
                    <div class="table-responsive pb-3">
                        <table class="table table-sm table-hover m-0">
                            <tbody>
                                <template v-if="Object.keys(data.items).length">
                                    <tr v-for="item in data.items">
                                        <td class="border-bottom align-middle w-100 py-2 px-1">
                                            {{ item.title_text }}
                                        </td>
                                        <td class="border-bottom align-middle text-right px-2">
                                            <b>{{ item.total }}</b>
                                        </td>
                                    </tr>
                                </template>
                                <template v-else>
                                    <tr>
                                        <td class="border-bottom align-middle w-100 text-danger py-2 px-1">
                                            Целей нет
                                        </td>
                                        <td class="border-bottom align-middle text-right text-danger px-2">
                                            <b>0</b>
                                        </td>
                                    </tr>
                                </template>
